...
$.widget("custom.vbooking", {
    ...
    *************************************************************************/
    ...
    _beginEvent: function () {
        ...
        $("body").unbind("fTripReady").on("fTripReady", function (e) {
            if (e.cb != undefined) {
        ...
        $("body").unbind("reloadSheet").on("reloadSheet", function (e) {
            var afReload = function () {
                if (e.cb) e.cb.call(this, e);
            }
            me._reloadSheet(afReload);
        });
        $("body").unbind("storeHistory").on("storeHistory", function (e) {
            me._storeHistory(e.d.un, e.d.key, e.d.his);
        });
        $("body").unbind("clearSelectedItem").on("clearSelectedItem", function (e) {
            me._clearSelectedItem();
        });
        $("body").unbind("doGetCurrentTripInfo").on("doGetCurrentTripInfo", function (e) {
            var tripInfo = me._getCurrentTripInfo();
            vbf('CurrentTripInfo', { tripInfo: tripInfo });
        });
        $("body").unbind("doResetAll").on("doResetAll", function (e) {
            me._resetAll();
        });
    },
    ...
    *************************************************************************/
    ...
    _reloadAllowedSeats: function () {
        var self = this;
        if (app.onlineCompId == null) return;
            ...
            $(obj).on('click', function (e) {//click len ghe
                ...
                if (seat._isAvailable()) {
                    if (!seat._hasTicket()) {
                        ...
                        //        var ind = app.seatStack.indexOf(seat);
                        ...
                        $(b).on('click', function (e) {
                            ...
                            var cb = function (u, r, l, t) {
                                ...
                                if (!app.fMoving) {
                                    ...
                                    //    }
                                    //}
                                    vbf('showUpdateForm', { self: me, obj: obj, seat: seat });
                                } else {
                                    if ($._isInMovingStack(seat)) {
                                        ...
                                        vbf('onResetMovingStack'); // reset moving stack
                                        vbf('showUpdateForm', { self: self, obj: obj, seat: seat }); // Show update form
                                    } else {
                                        vbf('showErrorForm', { message: _dict._err[3] }); // Show error form
                                    }
                                }
