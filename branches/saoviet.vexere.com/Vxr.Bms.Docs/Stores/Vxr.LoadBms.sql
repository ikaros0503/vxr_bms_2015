USE [VXR_BMS_003]
GO
/****** Object:  StoredProcedure [dbo].[LoadBms]    Script Date: 6/1/2015 6:08:27 PM ******/
SET ANSI_NULLS ON
GO
...
---------------------------
	...
	begin
		select top 1 @tId = Id from zgcl_Trip04 where CompId = @cId and IsPrgStatus != 3 and Type=1 ;
	end

	-- 1) Trip
	select Id, Name, Info, SeatTemplateInfo, FareInfo, StatusInfo, BaseId, Type, Time, Alias, TeamInfo, VehicleInfo, FromArea, ToArea, Note, RouteInfo, SeatSummaryInfo, TotalSeats, ClosedStatus, PassengerMoney, ProductMoney, FeeMoney, BranchReceiveProduct, RealDepartureTime, PickedPointsInfo, TransferPointsInfo, NewFare
		from zgcl_Trip04 where CompId=@cId and  (IsPrgStatus is null or IsPrgStatus != 3) and Info is not null and len(Info)>0 and (Type=1 or (Type=2 and Date = @date));

	insert into @tcTable (Id, Time) 
		(select Id, Time from zgcl_Trip04 where CompId=@cId and  (IsPrgStatus is null or IsPrgStatus != 3) and (Type=2 and Date = @date) and StatusInfo=2)
	...
	select Id,TripId,AgentId,TripDate,SeatCode,IssueDate,PickupDate,CustomerInfo,Status,Fare,Note,PickupInfo,Serial,PaymentInfo,IsPrgHistoryInfo,Code,PassCode,FromValid,ToValid,IsPrgUpdatedDate,AgentInfo,Surcharge,TripAlias,CreatedUser,UserCharge,StageCode,Deposit,Discount,FromArea,ToArea,SeatType,Debt,CanceledDate,CancelInfo,Type,ChargeDate,SNote,ResponsibilityUser,PickOrReturnDate
			from zgcl_Ticket03 where Status in (1,2,3,4,5,8) and TripDate = cast(@date as datetime) + cast(@tripTime as datetime) and TripId = @tId
