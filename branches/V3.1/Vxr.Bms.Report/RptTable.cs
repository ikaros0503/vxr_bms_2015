...
namespace Vxr.Bms.Report
{
    public class RptTable
    {
        ...
        public TreeNode<RptColumn> AddDataNode(TreeNode<RptColumn> parent, string hField, string hText, string hTable, string hRefField, string hRefText, string hRefTable, int level, bool isleaf, string type, string datatype, int index, int id, int iNotFillData)
        {
            RptColumn column = new RptColumn {
                cPattern = { hField = hField, hText = hText, hTable = hTable },
                cRef = { hRefField = hRefField, hRefText = hRefText, hRefTable = hRefTable },
        ...
        public string AddMakup(List<RptItem> list, int[] arrPos, int curIndex)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                for (int j = from; j < to; j++)
                {
                    ...
                    table.Append("</tr>");
                    if ((this.bShowSubSumary > 0) && (item.list.Count > 1))
                    {
                                    ...
                                    for (int m = 2; m < this.cCols; m++)
                                    {
                                        ...
                                        else if (this.SubRowSumGobal.position == m)
                                        {
                                            if (mColMergForSubSumRowSub > 0)
                                            {
                                                ...
                                                table.Append(string.Concat(new object[] { "<td class=\"", this.SubRowSumGobal.cssClassShowName, "\" colspan=", this.mColMergForSubSumRowSub, ">", this.SubRowSumGobal.showName, "</td>" }));
                                            }
                                            else
                                            {
        ...
        public void AddParentMakup(ref StringBuilder table, RptItem item, int cCountCols, object[] a, int[] arrPos, int curIndex, bool bAddSubSub)
        {
            ...
            if (bAddSubSub)
            {
                str = "<td class=\"rptTD\" rowspan=" + item.arrIndexData.Count.ToString() + ">";
            }
            if (((this.bShowSubSumary > 0) && (item.list != null)) && (item.list.Count > 1))
            {
                str = "<td class=\"rptTD\" rowspan=" + ((item.arrIndexData.Count + item.list.Count)).ToString() + ">";
            }
            table.Append(str);
        ...
        public void BuidlASimpleByDB(RptTable table, string tblName, string tblGobal, string reportId)
        {
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                if (table.bShowSTT)
                {
                    nodeArray[0] = table.AddDataNode(table.root, "", this.bShowSTTName, "", "", this.bShowSTTName, "", 0, true, "isData", "int", -1, -1, 1);
                    index = 1;
                }
                while (num < dataTable.Rows.Count)
                {
                    ...
                    if (str7.ToUpper().Contains("ISPARENT"))
                    {
                        ...
                        {
                            nodeArray[index] = table.AddDataNode(table.root, hField, hText, hTable, hField, hText, hTable, 0, false, "isData", datatype, -1, -1, iNotFillData);
                        }
                        else
                        {
                            nodeArray[index] = table.AddDataNode(table.root, hField, hText, hTable, hField, hText, hTable, 0, true, "isData", datatype, -1, -1, iNotFillData);
                        }
                    }
                    else
                    {
                        ...
                        if (str8.ToUpper().Contains("HASCHILD"))
                        {
                            nodeArray[index] = table.AddDataNode(nodeArray[num5], hField, hText, hTable, hField, hText, hTable, level, false, "isData", datatype, -1, -1, iNotFillData);
                        }
                        else
                        {
                            nodeArray[index] = table.AddDataNode(nodeArray[num5], hField, hText, hTable, hField, hText, hTable, level, true, "isData", datatype, -1, -1, iNotFillData);
                        }
                    }
                    index++;
                    num++;
