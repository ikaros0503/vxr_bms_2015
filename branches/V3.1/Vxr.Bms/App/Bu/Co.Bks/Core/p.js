...
$.widget("custom.vbooking", {
    ...
    *************************************************************************/
    ...
    _createGetSeatObj: function () {
        ...
        obj._c = {
            ...
            //Status: '(($x) IN(2, 3, 4, 5, 8))', //STATUS
            //ExpiredTime: "(Status = 1 AND ($x is null or $x >= '" + new Date().toFormatString("iso") + "'))"
            Status: "(($x) IN(2, 3, 4, 5, 8) or (Status = 1 AND (ExpiredTime is null or ExpiredTime >= '" + new Date().toFormatString("iso") + "')))", //STATUS
            //ExpiredTime: "(Status = 1 AND ($x is null or $x >= '" + new Date().toFormatString("iso") + "'))"
        };
    ...
    *************************************************************************/
    ...
    _createTicketFromRecord: function (record) {
        ...
        if (alias == self._cTripBus) { //Alias
            ...
            var re = new T(id, dept, cid, cname, cphone, status, issue, pdate, fare, note, pInfo, seri, pmInfo, hInfo, code, pcode, fromValid, toValid, updatedDate, aInfo, surCharge, suser, cuser, stageCode, tripId, deposit, discount, fromArea, toArea, seatType, debt, canceledDate, cancelInfo, type, chargeDate, sNote, responUser, porDate, staffName, cancelType);
    ...
    *************************************************************************/
    ...
    _bindEventOnSheet: function () {
        ...
        vev('.select-layout > button', 'click', function () {
            $('.select-layout > button').removeClass('active');
            $('button.btn-thong-ke').removeClass('active');
