...
$.loadModule = function (c, data, cb) {
    ...
    var load = function () {
        ...
        if (typeof ref == 'string' && ref)
            $(ref).load(f + "/v.html?v=" + version, function(x) {
                $.start(c, data);
...
$.hideAndShowTpl = function (c) {
    if (c[2] == 3 && c[12]) c[2] = c[12];
    var ref = c[2];
    if (typeof ref == 'string' && ref) {
        var pr = $(ref).parent();
        var childs = pr.children();
...
$.start = function (c, data) {
    ...
    require($.getJsFiles(c), function (x1, x2) {
        ...
        if (!o) o = {}; if (!p) p = {};
        o.p = p; o.c = c; o.appId = c[7]; //TODO: Need clone?
        o.data = data;
        $.ajaxSetup({ cache: true });
...
app.bms = { //Base modules
    ...
    BksMTicket: [0, 2, 0, 1, 0, 'Ticket Model', '', 'Bu/Co.Bks/Model/Ticket', '', '',0, 0, '', 0, 0, ''],
    BksCoreModule: [0, 0, 0, 1, 0, 'Core Bks', '', 'Bu/Co.Bks/Core', '', '', 0, 0, '', 0, 0, ''],
...
app.mds = {
    ...
    BksShippingMain: [0, 2, 3, 3, 0, 'Product Main', 'BksShippingCore', 'Bu/Co.Bks.Shipping/Main', 'S60000', 'a.btn-hang-hoa', 0, 1, '#product-content', 0, 1, '', '', '', ''],
    ...
    Cod: [0, 2, 3, 0, 0, 'Quản lí COD', '', 'Bu/Co.Bks/Module/Cod', '', 'a.cod-link', 0, 1, '#cod', 0, 1, '', '', 'cod', ''],
    OTicket: [0, 2, 3, 0, 0, 'Quản lý vé offline', '', 'Bu/Co.Bks/Module/OTicket', '', 'a.offline-ticket', 0, 1, '#oTicket', 0, 1, '', '', 'offline-tickets', ''],
    ...
    MDis: [0, 1, 1, 0, 0, 'Quản lý Chiết khấu', '', 'Manage/Co.Disc', '', 'a.discount', 0, 0, '', 0, 1, '', '', 'discount-manager', ''],
    ...
    MRoute: [0, 1, 1, 0, 0, 'Quản lý tuyến đường', '', 'Manage/Co.Bo.Rou', '', 'a.trip', 0, 1, '', 0, 0, '', '', 'route-manager', ''],
    MFare: [0, 1, 1, 0, 0, 'Quản lý giá vé', '', 'Manage/Co.Bo.Rou.Fare', '', 'a.fare', 0, 1, '', 0, 0, '', '', 'fare-manager', ''],
    MTrip: [0, 1, 1, 0, 0, 'Quản lý chuyến', '', 'Manage/Co.Bo.Rou.Trip', '', 'a.config-route', 0, 1, '', 0, 0, '', '', 'trip-manager', ''],
    ...
    BksReport: [0, 3, 3, 0, 0, 'Báo cáo', '', 'Bu/Co.Bks.Report', '', 'a.watch-report', 0, 1, '#report', 0, 1, 'xRptJs:[C]/report.js'],

    //Vxr Modules
    //TripSummary: [0, 1, 3, 0, 0, 'Tuyến xe', '', 'Bu/Co.Bks.Ag.Bu/WatchRoutes', 'S100660', 'a.watch-routes', 0, 1, '#watchRoutes', 0, 1, '', '', 'watch-routes', '']
    TripSummary: [0, 1, 3, 0, 0, 'Tuyến xe', '', 'Vms/VRouteTrips', 'S100660', 'a.watch-routes', 0, 1, '#watchRoutes', 0, 0, '', '', 'watch-routes', '']
};
app.rp = {
