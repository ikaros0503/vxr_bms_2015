...
(function ($) {
    $.extend(_dict, _dict,
    {
        ...
        [
			...
			[8, 2, "sNote", "input", "text", "SNote", "form-control", "", {}, [], ""],
			[9, 1, "Code", "input", "text", "Serial", "form-control", "", {}, [], ""],
		...
		// in phơi
        _pclTpl: "<div class='list'><h3>DANH SÁCH KHÁCH HÀNG</h3><table class='table table-bordered'><thead><tr><th>Số ghế</th><th>Tên HK</th><th>Code</th><th>Điểm đón</th><th>Số điện thoại</th><th>Đại lý</th><th>Ghi chú</th><th>sNote</th></tr></thead><tbody></tbody></table></div>",
        _ipcLTpl: "<tr class='{seat._class}'><td>{seat._seatCodes}</td><td>{seat._cname}</td><td>{seat._seri}</td><td>{seat._pText}</td><td>{seat._cphone}</td><td>{seat._pmInfoName}</td><td>{seat._note}</td><td>{seat._snote}</td></tr>",
