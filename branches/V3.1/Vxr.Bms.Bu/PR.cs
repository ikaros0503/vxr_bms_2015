...
namespace Vxr.Bms.Bu
{
    ...
    public class PR
    {
        ...
        public static void rp(object obj, out object oo)
        {
            ...
            var x = new XR(obj); XR.Init(_sc, IsDebug, DR._a, DR._h, DR._f, DR._fd);
            var confs = x._cf;
            var que = confs["query"] as ArrayList;
            if (que != null && (confs.ContainsKey("query") && que.Count > 0))
            {
                var sqlQuery = confs["query"] as ArrayList;
                if (sqlQuery != null)
                {
                    query = sqlQuery[0].ToString();
                }
