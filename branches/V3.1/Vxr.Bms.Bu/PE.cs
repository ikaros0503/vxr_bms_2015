    ...
    public class PE
    {
        ...
        public static void UpdateBookTicket(object obj, out object oo)
        {
            ...
            var r = x._CF().Pr("sql", SqlDbType.NVarChar).Pr("fields", SqlDbType.NVarChar).
                Pr("tableName", SqlDbType.NVarChar).Pr("ids", SqlDbType.NVarChar).C("sql", " DECLARE @ID AS INT; " + string.Join(";", sql)).C("tableName", "Ticket").C("fields", x.DT()).C("ids", string.Join(",", lstId)).VS("GetDataFirst").G();
            //var r = x.EX2(sql).G();
            if (r._s == 1)
            {
                var sq = new SendQueue();
