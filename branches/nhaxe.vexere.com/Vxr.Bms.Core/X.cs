...
namespace Vxr.Bms.Core
{
    ...
    public class X
    {
        ...
        public X L()
        {
            ...
            if (_cd != null) foreach (var c in _cd){if (c.Key.Contains("Password")){string pass = MD5Hash(c.Value.ToString()) + "/MD5Hash"; _c[_cc] = new[] { c.Key, L(c.Key, 7, 0, _fs.Length, _fs), pass + "" };}
                    else _c[_cc] = new[] { c.Key, L(c.Key, 7, 0, _fs.Length, _fs), c.Value + "" }; _cc++;}
            if (_da != null) foreach (var d in _da){if (d.Key.Contains("Password")){if (isNewPass(d.Value.ToString())) { string pass = MD5Hash(d.Value.ToString()) + "/MD5Hash"; _d.Add(new[] { d.Key, L(d.Key, 7, 0, _fs.Length, _fs), pass + "" }); }
                        else _d.Add(new[] { d.Key, L(d.Key, 7, 0, _fs.Length, _fs), d.Value + "" });}else _d.Add(new[] { d.Key, L(d.Key, 7, 0, _fs.Length, _fs), d.Value + "" });}
            return this;
        }
