define({
    ...
    _checkBeforeDelete: function (isUndel, cb) {
        ...
        $.each(c, function (k, v) {
            if (v == '$base') c[k] = me._getBaseModel().Id;
                ...
                me._applyCondition($.dx, name, c, k);
                ...
                $.each(pFields, function (idx, p) {
                    if (p.indexOf('$') == 0) {
                        ...
                        me._applyCondition($.dx, names, obj, names);
                    }
                });
    ...
    onBeginSelectionChange: function (view, record) {
        ...
        // console.log(record);
        if (record) {
            // view.find(selector).prop('disabled', false);
    ...
    afterReload: function () {
        $('body').unbind('clickdatve').on('clickdatve', function (e, trip, date, time) {
            e.preventDefault();
            $('select[name=TripId]').val(trip).trigger('change');
            $('input[name=DepartureDate]').datepicker('setDate', vPrsDt(date));
            $('select[name=TimeSlot]').val(time).trigger('change');
    ...
    checkAndSave: function (process) {
        ...
        if (typeof _dict._blockSeatByVehicle != "undefined") {
            var m = view.find('select.vehicle').val();
            var mText = view.find('select.vehicle option[value="' + m + '"]').text();
            var lsBlock = [];
