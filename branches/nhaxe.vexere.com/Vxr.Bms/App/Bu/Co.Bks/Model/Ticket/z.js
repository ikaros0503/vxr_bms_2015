...
**************************************************************************************/
var T = function (id, dept, cid, cname, cphone, status, issue, pdate, fare, note, pInfo, seri, pmInfo, hInfo, code, pcode, fromValid, toValid, updatedDate, aInfo, surCharge, suser, cuser, stageCode, tripId, deposit, discount, fromArea, toArea, seatType, debt, canceledDate, cancelInfo, type, chargeDate, sNote, responUser, porDate) {
    this._id = id;
    this._dept = dept;
    this._cid = cid;
    this._cname = cname;
        ...
        if (vIsEstStr(self._hInfo)) {
            ...
            for (var i = 0; i < h.length; i++) {
                ...
                if (vIsEstStr(ht[6])) {
                    ...
                    var dt = "";
                    $.each(d, function (k, v) {
                        if (_dict._historyKeys.hasOwnProperty(k)) {
