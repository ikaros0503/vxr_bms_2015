...
/*Render HTML *************************************************************************/
S.prototype._renderSeat = function (nTicketPerTrip, hasBPermit, cStageCode, numOfStages, stopPoints, isStageBooking, maxStageCode, closedStatus, realDepartureTime, tripDetailId, vehicleNumber, notallow, cTripId) {
    var self = this;
    //Check status of seat map with ticket, expected each seat map with one ticket
    var ticket = self._getCurrentTicket(cStageCode);

    // block seat by vehicle => set status of Seat = 2 (block)
    var seatPos = self._coach + "|" + self._row + "|" + self._col;
    ...
    if (self._isAvailable()) {//this._status == 1 ??
        //Prepare data
        ...
        var tooltip = self._renderStageTicketsInfo_Tooltips(self._tickets, stopPoints);
        if (!$.isEmptyObject(ticket)) {
            data["seat"]["_stageticketsTooltips"] = tooltip;
            data["seat"]["_numStagetickets"] = self._tickets.length;
            ...
            self._updatePaymentInfo_AgentInfo_ToSeat(data, ticket);
        ...
        if (self._tickets.length >= 1) {
            ...
            if ((stageCode & cStageCode) == 0) {
                ...
                var firstTicket = self._getFirstStageTicketInSeat();
                ...
                self._updatePaymentInfo_AgentInfo_ToSeat(data, firstTicket);
                //Agent Info
                if (vIsEstStr(ticket._suser)) {
                    data["seat"]["_suser"] = "B:&nbsp;" + ticket._suser;
                }
        ...
        if (self._tickets.length == 1 && !isStageBooking && (ticket.stageCode == cStageCode || cStageCode == 0)) {
        ...
        //Rendering
        if ($.isEmptyObject(ticket) && self._tickets.length <= 0) {
            //console.log("=======Không vé hiện hành và không vé trong ghế");
            $html = $(vtpl(_dict._sTpl, data));//Không tooltips
        } else if ($.isEmptyObject(ticket) && self._tickets.length >= 1) {
...
S.prototype._renderPrintSeat = function (nTicketPerTrip, stopPoints) {
    var self = this;
    //Prepare data
    ...
    if (!$.isEmptyObject(ticket)) {
        ...
        var pInfo = ticket._getPickupInfo();
        //if (!$.isEmptyObject(pInfo) && pInfo.type == 'P') {
...
/* Add ticket
 *************************************************************************/
S.prototype._addTicket = function (t, cStageCode) {
    var self = this; //Seat

    var index = self._tickets.push(t) - 1;
    if (self._tickets[index].stageCode & cStageCode) {
        self._currentTicketIndex = index; // If current StageCode == the StageCode of Ticket=> current Ticket
    }

    if (!isNaN(self._tickets[index].stageCode)) { //kiểm tra vé có mã chặng
        self._seatStatus = self._seatStatus | self._tickets[index].stageCode; //SeatStatus => all tickets booked
        //Đánh index cho vé có MaxStageCode
        if (index == 0 || self._tickets[index].stageCode >= self._tickets[self._tickets_idxMaxStageCode].stageCode)
