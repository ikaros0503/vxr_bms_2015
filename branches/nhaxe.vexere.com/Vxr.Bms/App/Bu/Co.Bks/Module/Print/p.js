...
(function ($) {
    ...
    $.extend(true, $.custom.vbooking.prototype, {
            ...
            printWindow.document.write(
                '<html>' +
                    '<head>' +
                        ...
                        '<link rel="stylesheet" href="' + this.options.baseUrl + _dict._pStyleUrlDefault + '" type="text/css" />' +
                        '<link rel="stylesheet" href="' + this.options.baseUrl + _dict._pStyleUrl + '" type="text/css" />' +
                    '</head>' +
                    '<body>' +
                        $printContent.html() +
                    '</body>' +
                                                        ...
                                                        if (fields.length > 0) {
                                                            for (var jj = 0; jj < fields.length; jj++) {
                                                                if (fields[jj].indexOf("phone") >= 0) {
                                                                    ...
                                                                    display.data.pInfo = t._pInfo.split('|')[1];
                                                                }
        ...
        _printPickupTicket: function () {
            var self = this;
            var $printContent = $('<div />').addClass('print-container')
                .append(self._renderBKSTitle()).append(self._renderPrintPickupTemplate());
            ...
            printWindow.document.write(
                '<html>' +
                    '<head>' +
                        '<title>' + windowName + '</title>' +
                        '<link rel="stylesheet" href="' + self.options.baseUrl + _dict._pStyleUrl + '" type="text/css" />' +
                    '</head>' +
        ...
        _printTransferTicket: function (groupIndex) {
            var $printContent = $('<div />').addClass('print-container')
                .append(this._renderBKSTitle()).append(this._renderPrintTransferTemplate(groupIndex));
        ...
        _renderPrintTransferTemplate: function (groupIndex) {
            var self = this;
            var tName = self._data[self._cTripIndex].Name;
            var group = self._$transfer[groupIndex];
            var totalSeat = 0;
            if (group._items.length > 0) {
                $.each(group._items, function (_, s) {
                    if (typeof s != "undefined" && s != null) {
            ...
            var $transferUp = $(vtpl(_dict._ptsfTpl_NoSub, { "t": { _total: group._total.toMn() + "đ", _totalSeat: totalSeat } }));
