    ...
    $.extend(true, $.custom.vbooking.prototype, {
        ...
        *************************************************************************/
        _create: function () {
            ...
            var self = this;
            $("body").on("updateFilters", function (e, model) {
        ...
        updateFilters: function (model) {
            ...
            self._changeCTripTime($("#FilterForm select[name=TimeSlot]").val());
        ...
        *************************************************************************/
            ...
            if (!self._hasBookReturnTicket) {
                if (FlatObj.LDefTrip) {
                    if (vIsEstStr(app.rights)) {
                        ...
                        if (app.rights.indexOf($.rights.bBookPastTk.val) >= 0) {
        ...
        _bindEventOnFilterForm: function () {
            ...
            $form.find('button').each(function (_, b) {
                if ($(b).hasClass('btn-print')) {
                    $(b).unbind().on('click', function (e) {
                        e.preventDefault();
                        $('body').trigger('fPrintExcuse', self);
                    });
            ...
            $form.find('select[name=TripId]').on('change', function () {
                ...
                self._changeCTripId(this.value);
                self._changeCTripIndex(this.selectedIndex);
                self._changeCFromId(0);
                self._changeCToId(0);
                self._reloadStopPoints();
            ...
            $form.find('select[name=FromPoint]').on('change', function () {
                ...
                self._changeCFromId(this.value);
                self.reloadAll();

            });
            $form.find('select[name=ToPoint]').on('change', function () {

                self._changeCToId(this.value);
                self.reloadAll();

            });
            $form.find('input[name=DepartureDate]').on('change', function () {
                // reset lại FTripGetTrip
                FlatObj.FTripGetTrip = false;
                //Không cần reload lại StopPoints...
                var departureDate = this.value.split(' ');
                self._changeCTripDate(vPrsDt(departureDate[departureDate.length - 1]));
                //Reset current timeslot and bus
                if (!self._hasSearchPhone) {
                    self._cTripTime = null;
                    self._cTripBus = 0;
                }
                self.reload();
            ...
            //update by Thanh
            $form.find('a[name=PreDate]').on('click', function () {
                var date = $("#FilterForm input[name=DepartureDate]").datepicker('getDate');
                date.setDate(date.getDate() - 1);
                //alert(date.toFormatString('DD dd-mm-yyyy'));
                $("#FilterForm input[name=DepartureDate]").datepicker('setDate', new Date(date));
                $("#FilterForm input[name=DepartureDate]").trigger('change');
            });
            $form.find('a[name=NexDate]').on('click', function () {
                var date = $("#FilterForm input[name=DepartureDate]").datepicker('getDate');
                date.setDate(date.getDate() + 1);
                //alert(date.toFormatString('DD dd-mm-yyyy'));
                $("#FilterForm input[name=DepartureDate]").datepicker('setDate', new Date(date));
            ...
            //End Update
            $form.find('select[name=TimeSlot]').on('change', function () {
                //Không cần reload lại StopPoints...
                self._changeCTripTime(this.value);
                //self._reloadBus();
                self._reloadFrame();
                self._reloadSheet();
            ...
            $form.find('select[name=Bus]').on('change', function () {
                self._changeCTripBus(this.value);
                self._reloadFrame();
                self._reloadSheet();
            ...
            //self._$searchTicketFilter = $form.find('input[name=Keyword]');
            self._$searchTicketFilter = $('input[name=Keyword]');
            self._$searchTicketFilter
                .focus(function () {
                    if (!self._fMoving) {
                        //Reset seat stack
                        self._clearSelectedItem();
                ...
                .keyup(function (e) {
                    if (this.value.length == 6) {  //search code
                        e.preventDefault();
                        e.stopPropagation();
                        self._searchTicket(this.value, true);
            ...
            self._$searchTicketFilter.bind('paste', function (e) {
                ...
                if ((this.value.length == 6 || this.value.length == 10 || this.value.length == 11)) {
                    ...
                    if (this.value.length == 6) {
                        ...
                        if (!isNaN(this.value)) self._searchTicket(this.value);
                    }
                } else {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                self._$searchTicketFilter.blur();
            });
        },
        ...
        _bindEventOnSearchResult: function () {
            ...
            if (self._totalSearchResult > 0) {
                ...
                li.on('click', function (e) {
                    ...
                    self._$filterForm.find('input[name=DepartureDate]').val(vPrsDt(tripDateRecord).toFormatString('DD dd-mm-yyyy'));
        ...
        *************************************************************************/
        ...
        _reloadAvailableSeat: function () {
            ...
            var completeGetTripTotalSeatInfo = function (u, r, l, t) {
                if (u != 1) return;
                self._mapAvailableSeat(r);
            };
            //Submit query
            self._submitAsyncAction(obj, completeGetTripTotalSeatInfo);
        },
