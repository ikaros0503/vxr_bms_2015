USE [VXR_BMS_003]
GO
/****** Object:  StoredProcedure [dbo].[LoadBms]    Script Date: 6/13/2015 11:18:45 AM ******/
SET ANSI_NULLS ON
GO
...
---------------------------
	...
	begin
		select top 1 @tId = Id from zgcl_Trip04 where CompId = @cId and IsPrgStatus != 3 and Type=1 order by type,name asc;
	end

	-- 1) Trip
	select Id, Name, Info, SeatTemplateInfo, FareInfo, StatusInfo, BaseId, Type, Time, Alias, TeamInfo, VehicleInfo, FromArea, ToArea, Note, RouteInfo, SeatSummaryInfo, TotalSeats, ClosedStatus, PassengerMoney, ProductMoney, FeeMoney, BranchReceiveProduct, RealDepartureTime, PickedPointsInfo, TransferPointsInfo, NewFare, Description
		from zgcl_Trip04 where CompId=@cId and  (IsPrgStatus is null or IsPrgStatus != 3) and Info is not null and len(Info)>0 and (Type=1 or (Type=2 and Date = @date)) order by type,name asc;
