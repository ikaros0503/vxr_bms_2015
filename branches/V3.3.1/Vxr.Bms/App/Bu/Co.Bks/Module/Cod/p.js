define({
    ...
    fieldsToGet: [ "Code","Type",  "Status","TripDate","SeatCode", "IssueDate","PickupDate","CustomerInfo","Fare","Note", "PickupInfo", "PaymentInfo",
        ...
        "IsPrgCreatedDate", "CancelType", "TripName", "ChargeDate", "Id", "BookingCode", "TransactionStatus", "Source", "AddressInfo","FromAddress","PaymentMethod"],
    smsTemplate: "Kinh gui QK ve xe tuyen {tripName}. Ma Ve: {code}, Ghe: {seat}, Xe: {compName}, Noi di: {fromAddress} luc {tripTime} {tripDateSms}. Tran Trong. VeXeRe. Hotline: 19006484",
    ...
    init: function () {
        ...
        var bindEvents = function () {
            vev("#cod button.refresh", 'click', function () {
                self.loadTickets();
            });

            vev(".btnChangeCodStatus", 'click', function () {
                ...
                vRqs({
                    _a: 'UpdateTicketStatus',
                    bookingCode: self.tickets[index].bookingCode,
                    actor: app.un,
                    transactionStatus: newStatus,
                    paymentMethod: self.tickets[index].paymentMethod,
            ...
            vev("#cod input", 'change', function () {
                self.updateFilter().loadTickets();
            });

            vev("#cod select", 'change', function () {
                self.updateFilter().loadTickets();
            });

            vev("#cod .mnuCodPayment li a", 'click', function () {
                if (!$(this).hasClass('selected')) {
                    $("#cod .mnuCodPayment li a").removeClass("selected");
                    $(this).addClass('selected');
                    $("#mnuCodPayment span.content").text($(this).text() == 'Tất cả' ? 'Thanh toán' : $(this).text());
                    self.updateFilter().loadTickets();
                }
            });

            vev("#cod .mnuCodStatus li a", 'click', function () {
                if (!$(this).hasClass('selected')) {
    ...
    updateFilter:function() {
        ...
        var data = {
            ...
            payment: $("#cod .mnuCodPayment li a.selected").attr('data-payment'),
        ...
        self.filter +=(data.payment!="" &&data.payment!= "-1")?(' and PaymentMethod=' + data.payment):" and (PaymentMethod=1 or PaymentMethod=2)";
    ...
    loadTickets: function () {
        ...
        var getTicketFromRecord = function (record, index) {
            ...
            result.tripDateSms = moment(tripDate).format('DD-MM-YY');
            ...
            result.payment = record[30] == "1" ? "COD" : "Tại văn phòng";
            result.paymentMethod =parseInt( record[30] );

            result.status = self.statusArr[record[26]];
            result.status2Display =  result.statusId == 1?"":"hide";
            result.status3Display =  (result.statusId == 2 && result.payment != "Tại văn phòng") ? "" : "hide";
            result.status4Display = (result.statusId == 3 || (result.payment == "Tại văn phòng" && result.statusId == 2)) ? "" : "hide";
            result.status5Display = result.statusId == 1 || (result.statusId == 3 &&result.payment == "COD") ? "" : "hide";
            ...
            if (result.payment == "COD") {
                ...
                result.status2Display = result.statusId == 1 ? "" : "hide";
                result.status3Display = "hide";
                result.status4Display = result.statusId == 2 ? "" : "hide";
                result.status5Display = (result.statusId == 1 || result.statusId == 2) ? "" : "hide";
            }
    ...
    renderTickets: function () {
        ...
        var bindEventToRow = function () {
            ...
            vev("#cod .mnuCodStatusRow li a", 'click', function () {
                if (!$(this).hasClass('selected')) {
                    var index = parseInt($(this).attr("data-index"));
            ...
            vev("#cod .btn-update-status", 'click', function () {
                var index = parseInt($(this).attr("data-index"));
                var newStatusId = $(this).attr("data-status");
