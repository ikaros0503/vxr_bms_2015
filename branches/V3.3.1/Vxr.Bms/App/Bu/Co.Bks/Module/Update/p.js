...
define({
    ...
    _createUpdateDialogDiv: function () {
        var self = this;
        var areaObj = self._createGetAreaObj();
        vRqs(areaObj, function (u, r, l, t) {
            if (u != 1 || l <= 0) return;
            self._listArea = [];
    ...
    _createChosenSelectBox: function () {
    ...
    _createBaseIdQueryCondition: function (compId) {
        ...
        query = query + (" or ($x is null and CompId=" + compId + "))");
        return query;
    },
    ...
    _createGetAreaObj: function () {
        var me = this;
        var obj = {};
        var baseIdQuery = me._createBaseIdQueryCondition(app.cid);
        obj._a = "fGetArea";
        obj._c = {
            BaseId: baseIdQuery,
            Type: " $x=5 or $x=3 ",
    ...
    _createUpdateTicketObj: function (f, stopPoints, tBus, tId, offsetMinute, tDate, isPrintETicket) {
        ...
        $.each(app.seatStack, function (i, v) {
            ...
            if (vIsEstStr(fromStopSave)) {
                dObj.FromStop = fromStopSave;
            }
            if (vIsEstStr(toStopSave)) {
    ...
    _bindEventOnUpdateForm: function (f, st, tId, tName, tDate, tBus, sc, stopPoints, offsetMinute, cTrip, tdid, lmDate, cs, lpp, ltp, tTime, tInfo, blockEnterEvent) {
        ...
        f.find('input[type=text]').not('[readonly]').on('keypress', function (e) {
...
$(document).ready(function () {
    $._convertFormDataToObj = function (f) {
        ...
        if (app.hasCopyTicket || app.hasBookReturnTicket) isGetBlock = true;
        var inForms = f.find('#UpdateForm input,textarea,select');
        $.each(inForms, function (i, e) {
            if (!$(e).is(':disabled') || isGetBlock) {
