...
define({
    start: function (o) {
        try {
            var me = this;
            vbv('collectDataForPrintETicket', function (e) { // collect data for print eticket
                if (e.d) {
                    vbf('onGetCurrentTripInfo');
                }
            });
            vbv('doPrintETicket', function (e) { // do print eticket
                if (e.d) {
                    me._creatPrintETicketContainer();
                    me._creatPrintETicketBody();
                    me._printETicket(e.d.tInfo, e.d.cTrip, e.d.tId, e.d.tName, e.d.tDate);
                }
            });
        } catch (excep) {
            console.error(excep);
    ...
    _printETicket: function (tIf, cTrip, tId, tName, tDate) {
        ...
        var $printContent = $('<div />').append(me._$printETicketContainer);
        ...
        printWindow.document.write(
            '<html>' +
                '<head>' +
                    ...
                    '<link rel="stylesheet" href="' + _dict._eStyleUrl + '" type="text/css" />' +
                '</head>' +
                '<body>' +
                    $printContent.html() +
                '</body>' +
                ...
                if (!$.isEmptyObject(t)) {
                    ...
                    data.push({
                        "ticket": {
                            ...
                            _vehicleNumber: tIf._vehicleNumber ? tIf._vehicleNumber : ""
                        }
                    });
                }
            });
