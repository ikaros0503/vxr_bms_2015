                        ...
                        $.each(cx, function (_, s) {
                            if (typeof s != "undefined" && s != null) {
                                if (s._hasTicket()) {
                                    ...
                                    if (t._type != 2) {
                                        // không hiển thị các vé đã hủy với lý do Hủy hoàn toàn
                                        if (t._cancelType != 4) {
                                            var nTicketPerTrip = 0;
                                            if (!$.isEmptyObject(t) && typeof self._cphoneSt[t._getDefaultPhoneNumber()] != "undefined") {
                                                nTicketPerTrip = self._cphoneSt[t._getDefaultPhoneNumber()].length;
        ...
        *************************************************************************/
        ...
        _renderTransferTicket: function () {
            ...
            if (transfer.length > 0) {
                var stickets = [], nstickets = [], nstickets2 = [], customer = [];
                var total = 0, totalSeat = 0;
                ...
                $.each(result, function (inOb, ob) {
                    var $transferUp = $(vtpl(_dict._tsfTpl, { "t": { _timeSlots: ob._times.join(', '), _total: ob._total.toMn() + "đ", _index: ob._index, _totalSeat: totalSeat } }));
