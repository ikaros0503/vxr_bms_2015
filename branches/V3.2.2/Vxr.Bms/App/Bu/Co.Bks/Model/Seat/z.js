///#source 1 1 /App/Bu/Co.Bks/Model/Seat/o.js
/************************************************************************
* ITEM                                                                  *
*************************************************************************/
...
///#source 1 1 /App/Bu/Co.Bks/Model/Seat/p.js
/*Render HTML *************************************************************************/
S.prototype._renderSeat = function (nTicketPerTrip, hasBPermit, cStageCode, numOfStages, stopPoints, isStageBooking, maxStageCode, closedStatus, realDepartureTime, tripDetailId, vehicleNumber, notallow, cTripId, limitedDatebyUser, isVMSLSeat) {
    var me = this;
    ...
    if (vIsEstStr(ticket._fromStop)) {
        var indFromStop = ticket._fromStop.indexOf('~');
        var fromStops = ticket._fromStop.substr(indFromStop + 1, ticket._fromStop.length).split("|");
        fromStop = {
            Id: fromStops[0],
            Type: fromStops[1],
            Code: fromStops[2],
    ...
    if (vIsEstStr(ticket._toStop)) {
        var indToStop = ticket._toStop.indexOf("~");
        var toStops = ticket._toStop.substr(indToStop + 1, ticket._toStop.length).split("|");
        toStop = {
            Id: toStops[0],
            Type: toStops[1],
            Code: toStops[2],
    ...
    if (me._isAvailable()) {//this._status == 1 ??
        ...
        if (limitedDatebyUser) {
            ...
            if (!$.isEmptyObject(ticket)) {
                ...
                //if (ticket._status == 1) {
                //    if (vIsEstStr(ticket._expiredTime) && vPrsDt(ticket._expiredTime) < new Date()) {
                //        data["seat"]["_class"] = "cancelled";
                //    } else {
                ...
                // First User Updated
                if (vIsEstStr(ticket._firstUserUpdated)) {
                    data["seat"]["_fUserUpdated"] = "B:&nbsp;" + ticket._firstUserUpdated;
                }
                ...
                if (isVMSLSeat != undefined && isVMSLSeat) {
                    if (ticket._type == 3) {
                        data["seat"]["_suser"] = "B:&nbsp;admin.vxr";
                        data["seat"]["_cuser"] = "&nbsp;/&nbsp;P:&nbsp;admin.vxr";
                        data["seat"]["_class"] = "vxr";
                    }
                }
            }
            ...
            if (hasBPermit) {
                ...
                //if (ticket._type != 2 || (ticket._type == 2 && app.rights.indexOf($.rights.bEdtOnlTic.val) != -1)) {

                //Khóa button của vé hờ và vé khác
                if (ticket._type != 2) {
                    $.each(_dict._at, function (j, a) {
                        if (st.indexOf(j) != -1) {
                            var isShow = true;
                            if (closedStatus == 2 || isVMSLSeat) {
                                isShow = false;
                            } else {
                    ...
                    // has permisson edit online tickets or move online tickets

                    if (app.rights.indexOf($.rights.bEdtOnlTic.val) != -1) {
                        $.each(_dict._at, function (j, a) {
                            if (st.indexOf(j) != -1) {
                                var isShow = true;
                                if (closedStatus == 2) {
                                    ...
                                    if (a[1] == "U") {
                                        if (app.rights.indexOf($.rights.bUpdate.val) == -1 || app.rights.indexOf($.rights.bUpOl.val) == -1) {
                                            isShow = false;
                                        }
                                    }
                                    if (a[1] == "M") {
                                        if (app.rights.indexOf($.rights.bMove.val) == -1 || (!_dict._stageTicketshowMButton && isStageBooking)) {
                                            isShow = false;
                                        }
                                        if (closedStatus == 1 && tripDetailId) {
                                            if (app.rights.indexOf($.rights.bEnBtnAtDpt.val) == -1) {
...
S.prototype._renderPrintSeat = function (nTicketPerTrip, stopPoints, displayData) {
    ...
    if (vIsEstStr(ticket._fromStop)) {
        var indFrom_1 = ticket._fromStop.indexOf('~');
        var from_1 = ticket._fromStop.substr(indFrom_1 + 1, ticket._fromStop.length).split("|");
        fromStop = {
            Id: from_1[0],
            Type: from_1[1],
            Code: from_1[2],
    ...
    if (vIsEstStr(ticket._toStop)) {
        var indTo_1 = ticket._toStop.indexOf("~");
        var to_1 = ticket._toStop.substr(indTo_1 + 1, ticket._toStop.length).split("|");
        toStop = {
            Id: to_1[0],
            Type: to_1[1],
            Code: to_1[2],
    ...
    data["pseat"]["_fromStop"] = fromStop;
    data["pseat"]["_toStop"] = toStop;
...
S.prototype._renderLSeat = function (nTicketPerTrip, index, hasBPermit, tic, stageName) {
    ...
    if (self._isAvailable()) {
        ...
        if (!$.isEmptyObject(ticket)) {
            ...
            data["seat"]["_cphone"] = ticket._getDefaultPhoneNumber();
            data["seat"]["_idate"] = ticket._issue ? ticket._issue.toFormatString('dd-mm-yyyy hh:ii') : "";
            //Agent Info
            ...
            data["seat"]["_issue"] = ticket._issue ? ticket._issue.getTime() : "";
        }
        if (hasBPermit && !app.oRights["StageEnable"]) {
            ...
            //TODO: tạm thời không hỗ trợ chức năng dạng danh sách
...
S.prototype._getSeatId = function () {
    var self = this;
    return "seat_" + self._coach + '_' + self._row + '_' + self._col;
};
