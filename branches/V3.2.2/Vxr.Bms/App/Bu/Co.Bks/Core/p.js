...
$.widget("custom.vbooking", {
    ...
    *************************************************************************/
    ...
    reload: function (args) {
        ...
        if (typeof me._reloadFilter === 'function') {
            ...
            me._reloadFrame();
            var cbReloadAllowedSeats = function () {
            ...
            me._reloadAllowedSeats(cbReloadAllowedSeats);
            me._isOrderring = false;
            //self._reloadFlatObj();
            //var obj = me._createLSeatObj();
        } else {
    ...
    *************************************************************************/
    _reloadSheet: function (afterReloadSheet) {
        ...
        app.oldData.t = me._cTripId;
        app.oldData.d = me._cTripDate.toFormatString('dd.mm.yyyy');
        app.oldData.h = me._cTripTime.replace(":", ".");
    ...
    _reloadAllowedSeats: function (cb) {
        ...
        if (app.onlineCompId.indexOf(parseInt(app.cid)) != -1) {
            ...
            var complete = function (u, r, l, t) {
                ...
                if (l >= 1) {
                    self._mapDataAllowedSeat(r, cb);
                } else {
                    if (cb != undefined) cb.call();
                    self._allowBookAllSeat = true;
                }
            }
            vdc.gBusTicketStatus(objA._c, '#', !FlatObj.LFTime, null, complete);
        } else {
            if (cb != undefined) cb.call();
        }

        var obj = this._createLSeatObj();
        vRqs(obj, function (u, r, l, t) {
            if (u == 1) {
                app.vmsLSeats = [];
                $.each(r, function (_, data) {
                    ...
                    app.vmsLSeats.push(item);
                });
            }
        });
    },

    _createLSeatObj: function () {
        var self = this;
        var obj = {
            _a: 'fGetBus_Tickets_Status_01',
            _c: {
                XTypeId: 11,
    ...
    _mapDataAllowedSeat: function (records, cb) {
        var self = this;
        app.vmsLSeats = [];
        self._allAllowedSeats = [];
    ...
    _checkAllowedSeats: function () {
        ...
        if (self._allAllowedSeats != undefined && self._allAllowedSeats.length > 0) {
            ...
            stop = false;
            if (app.vmsLSeats != undefined && app.vmsLSeats.length > 0) {
                $.each(app.vmsLSeats, function (_, v) {
                    if (!stop) {
                        if (!$.isEmptyObject(v.Date)) {
                            if (vGetDateOnly(v.Date) == vDtToStr('dd-mm-yyyy', self._cTripDate)) {
                                //console.log('Block seat', 'Same date');
    ...
    *************************************************************************/
    _renderSeatTemplate: function () {
        ...
        //Add by An
        var vmsLSeatsInfo;
        if (app.vmsLSeats.length > 0) {
            for (var i = 0; i < app.vmsLSeats.length; i++) {
                if (app.vmsLSeats[i]['Time'] == self._cTripTime && app.vmsLSeats[i]['Date'].toDateString() == self._cTripDate.toDateString()) {
                    vmsLSeatsInfo = app.vmsLSeats[0]['Info'];
                    break;
                }
            }
        }
                    ...
                    if (typeof rx != "undefined") {
                        $.each(rx, function (kx, s) {
                            if (typeof s != "undefined" && s != null) {
                                ...
                                var isVMSLSeat = false;
                                var vmsLSeat = s._label + '|' + s._coach + '|' + s._row + '|' + s._col;


                                if (vmsLSeatsInfo != undefined && vmsLSeatsInfo.indexOf(vmsLSeat) != -1) {
                                    isVMSLSeat = true;
                                }

                                var $seat = s._renderSeat(nTicketPerTrip, hasBPermit, self._cStageCode, numOfStages, self._data[self._cTripIndex].StopPoints, isStageBooking, self._cMaxStageCode, closedStatus, '', tripDetailId, vehicleNumber, notallow, self._cTripId, self._hasBookingPermissionByUsers(), isVMSLSeat);
                                if (app.fMoving) {
                                    var isMoving = false;
    ...
    _createTicketFromRecord: function (record) {
        ...
        if (alias == self._cTripBus) { //Alias
            ...
            var issue = vIsEstStr(record[5]) ? vPrsDt(record[5]) : null;
                        ...
                        vev(b, 'click', function (e) {
                            ...
                            if (!app.fMoving) {
                                ...
                                if (app.hasCopyTicket) {
                                    ...
                                    groupTicks = groupTicks.getUnique();
                                    var errPayment = false;
                                    if (groupTicks.length > 1) {
                                        errPayment = true;
                                        vbf('showErrorForm', { message: _dict._err[9] }); // Show error form
                                    }
                                    if (!errFare && !errPayment) {
                                        vbf('onQuickPayTicket', {
                                            // quick pay ticket
                                            tId: me._cTripId,
                                            tName: me._data[me._cTripIndex].Name,
                                            tDate: me._getDepartureTime(),
