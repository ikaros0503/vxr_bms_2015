define({
    ...
    afterReload: function () {
        ...
        if (!jQuery.isEmptyObject(app.oldData)) {
            $('select[name=Time]').find('option:contains('+app.oldData.h.replace('.',':') +')').attr('selected','selected');
        }
    },
    ...
    onClickAddTime: function (btn, args) {
        ...
        if (isValid) {
            ...
            var _t = $('select[name="Name"]').find('option:selected').val();
            var _d = $('input[name="Date"]').val();
            var _h = $('input:text[name="AddTime"]').val();
