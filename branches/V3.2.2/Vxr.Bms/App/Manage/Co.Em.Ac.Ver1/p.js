define({
    ...
    _loadRights: function () {
        ...
        var fs = obj['_f'].split(',');
        var html = '';
        var numColBKS = 8, numColRPT = 8, numColMGT = 8, numColNull = 8;
        if (me._listRights.length <= 0) {
            vRql(obj, {
                a: function (u, r, l, t) {
                },
                m: function (i, d) {
                    ...
                    me._listRights.push(model);
                },
                z: function (u, r, l, t) {
                    me.renderRightsTable();
                }
            });
        }
    },
    ...
    renderRightsTable: function () {
        var me = this;
        var html = '';
        var cols = 8;
        var groups = me._listRights.gtArr({ Type: 0 }, false);
        $.each(groups, function (_, g) {
            var tbody = me.view.find('#' + g.Code);
            tbody.html('');
            var rs = me._listRights.gtArr({ RoleGroup: g.Code }, false);
            var c = 1;
            var tr = $('<tr />')
                        ...
                        .append($('<td />').append($('<div />').addClass('checkbox').append($('<label />').attr('data-code',g.Code).append($('<i />').addClass('glyphicon glyphicon-ok-circle clc')).append($('<b />').addClass('text-primary').html(' Chọn hết'))
                            .on('click', function () {
                                me.view.find('#mycollapse' + ($(this).attr('data-code') ? $(this).attr('data-code'):'null')).find('input.Rs:checkbox').prop('checked', true);
                            }))))
                        .append($('<td />').append($('<div />').addClass('checkbox').append($('<label />').attr('data-code',g.Code).append($('<i />').addClass('glyphicon glyphicon-remove-circle clc')).append($('<b />').addClass('text-primary').html(' Bỏ chọn hết'))
                            .on('click', function () {
                                me.view.find('#mycollapse' + ($(this).attr('data-code') ? $(this).attr('data-code') : 'null')).find('input.Rs:checkbox').prop('checked', false);
                            }))));
            tbody.append(tr);
            tr = $('<tr />');
    ...
    onBeginSelectionChange: function (view, record) {
        ...
        v.find('input.Rs:checkbox').prop('checked', false);
        if (record) {
            ...
            if (s.length > 1) {
                $.each(s, function (_, val) {
                    ...
                    if (r.length == 3) {
                        ...
                        v.find('input.Rs:checkbox[value="' + r[1] + '"]').prop('checked', true);
                    }
                });
            }
        }
    ...
    svcInfo: function (x, v) {
        ...
        for (i = 0; i < me._listRights.length; i++) {
            var tb = me.view.find('input.Rs:checkbox[value="' + me._listRights[i].Id + '"]');
            if (tb.prop('checked')) {
                var r = me._listRights[i].Type + '|' + tb.prop('value') + '|' + me._listRights[i].App;
                info.push(r);
            }
        }
    ...
    gDeactivate: function () {
        ...
        if (is_checked === true) checkbox.val(2); else checkbox.val(1);
    },
    ...
    svcactive: function () {
        ...
        /// </summary>
        var me = this, v = me.view;
        var info = v.find('input[name="IsPrgStatus"]').val();
        return info;
    },
    rvcactive: function (x, v) {
        ...
        if (vIsEstStr(v)) {
            if (v === 1) { checkbox.prop('checked', false); checkbox.val(1); } else { checkbox.prop('checked',true); checkbox.val(2); }
        }
        return v;
    },
    //#endregion
