...
namespace Vxr.Bms.Queue
{
    class QueueReader
    {
        ...
        private static bool ComparePhone(string p1, string p2)
        {
            if (p1.Contains(" ")) return p1 == p2;
            if (!p2.Contains(" ")) return false;
            return p2.Split(' ')[1] == p1;
        }
                ...
                foreach (string id in lstId)
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(cif) && cif.Length > 10)
                        {
                            ...
                            try
                            {
                                var ctm = bdb.Customers.FirstOrDefault(b => b.CompId == compId && ComparePhone(b.Phone, phone));
                                if (ctm != null)
                                {
                                    cusId = ctm.Id;
