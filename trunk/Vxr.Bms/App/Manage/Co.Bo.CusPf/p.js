define({
    ...
    fromDate: '', toDate: '', cusId: '', cusPhone: '',
    ...
    onVMLoaded: function () {
        ...
        this.fromDate = app.cusFromDate;
        this.toDate = app.cusToDate;
        this.cusId = app.cusId;
        this.cusPhone = app.cusphone;
    ...
    loadCustomer: function () {
        ...
        var self = this;
        if (self.cusId == null && self.cusPhone == null) return;
        var c = {};
        if (self.cusId != null) {
            c = { Id: self.cusId, CompId: app.cid }
        } else {
            c = { Phone: self.cusPhone, CompId: app.cid }
        }
    ...
    getTrips: function (type) {
        ...
        var $title = self.view.find("h4.modal-title2");

        var dateStr = "";
        if (self.fromDate != null && self.fromDate != '' && self.toDate != null && self.toDate != '') {
            dateStr = " and TripDate>='" + self.fromDate + "' and TripDate<='" + self.toDate + "' ";
            $title.text("Thông tin khách hàng (" + self.fromDate + " → " + self.toDate + ")");
        } else {
        ...
        for (var i = 0; i < arr.length; i++) {
            $(tbd).append(
                vtpl(this.o.jHtml.ticketRow, {
                    code: arr[i][2],
                    route: arr[i][3],
                    pickupDate: arr[i][4] != null ? this.toDateString(moment(parseInt(arr[i][4].substring(arr[i][4].indexOf('(') + 1).replace(')/', '')))) : '',
                    seat: arr[i][5] != null ? arr[i][5].split('|')[0] : '',
                    money: vToMn(arr[i][6]),
                    issueDate: arr[i][8] != null ? this.toDateString(moment(parseInt(arr[i][8].substring(arr[i][8].indexOf('(') + 1).replace(')/', '')))) : '',
