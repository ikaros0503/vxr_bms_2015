define({
    start: function (o, cb) {
        ...
        try {
            var d = o.data;
            console.log(d);
            if (d && d.CompId) o.queryConditions.XCompanyId = d.CompId;
                //v.find('select.xCompId').val(d.CompId ? parseInt(d.CompId) : 0);
            if (cb) cb.call(this);
        } catch (e) {
        ...
        if (d && d.BaseId) {
            ...
            window.setTimeout(function () {
                ...
                if (d.Time) {
                    ...
                    if(fTime) fTime.val(d.Time);
                    if (fTime) fTime.trigger('change').trigger('chosen:updated');
                }
            }, 100);
        }
    },
    ...
    _onSave: function () {
        ...
        //For API only
        var pairArea = [];
        $.each(me.listAreaPair, function (_, x) {
            pairArea.push(x.FrId + "-" + x.ToId);
        });
        var dataApi = {
            XTypeId: tyId,
            XStatus: 1,
            ...
            XTime: xtime.join("|"),
            Info: info,
            StopPoints: pairArea.join("|"),
            TimeLimit: tL,
            IsPrgStatus: 1
        };
        me._saveData(data, cond, dataApi, me.o.updateAction);
    },
    _saveData: function (dt, cd, da, action) {
        var me = this, o = me.o, view = o.view;
        var obj;
        var pageSize = parseInt($('#' + o.gId + ' .jtable-page-size-change select').val());
        obj = { _a: action, _c: cd, _d: dt, _da: da };
    ...
    _onDelete: function () {
        ...
        me._saveData(data, cond, dataApi, me.o.removeAction);
    },
    ...
    onVMLoaded: function () {
        ...
        var d = me.o.data;
        if (d) {
            if (d.Date) v.find('.xDate').val(d.Date);
        ...
        /*if (d && d.Type == 'Trip') {
            console.log('onVMLoaded: ', d.CompId ? parseInt(d.CompId) : 0);
            setTimeout(function() {
               // v.find('select.xCompId').val(d.CompId ? parseInt(d.CompId) : 0);
            }, 5000);
        }*/
