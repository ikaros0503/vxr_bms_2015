define({
    ...
    getTicketInfo: function() {
        var self = this;
        var date = new Date($(".departure").datepicker("getDate"));
        var ndate = new Date(date);
        ndate.setDate(ndate.getDate() + 1);
        vRqs({
            _a: "fGetTicketSummary",
            _c: { CompId: "$x=" + app.cid + " and TripDate>='" + self.toDbDateString(date) + "' and TripDate<'" + self.toDbDateString(ndate) + "'" },
            _f: "TripDate, TripId, CompId, Booked, Paid, Total",
        }, function(u, r, l) {
            if (u == 1 && r.length > 0) {
                for (var i = 0; i < r.length; i++) {
                    var time = moment(r[i][0]).format('HH:mm');
    ...
    loadRoutesAndTrips: function() {
        ...
        vRqs({
            ...
            _c: { CompId: "$x=" + app.cid + " and IsPrgStatus=1 and (Type=1 or (Type=2 and Date='" + dateStr + "')) order by Type, DisplayOrder, Name" },
            _f: "Id, Type, Name, Date, Time, SeatTemplateInfo, Info, CompId, IsPrgStatus, BaseId,StatusInfo,VehicleInfo, TeamInfo, Note, FromArea, ToArea, RouteInfo, FareInfo,Keywords, TotalStage,BranchReceiveProduct, FromId, ToId, RouteId, DisplayOrder",
            ...
            var getTripInfoFromRecord = function (record, time) {
                ...
                var tripDetail = {
                    ...
                    tripType: record[10]==2?"enhanced":record[10]==3?"cancelled":"",
            ...
            if (u == 1 && r.length > 0) {
                ...
                //update status
                for (var rr in self.routesData) {
                    if (self.routesData.hasOwnProperty(rr)) {
                        var info=(self.routesData[rr].info);
                        for (var kk in self.routesData[rr].trips) {
                            if (self.routesData[rr].trips.hasOwnProperty(kk)) {
                                var trip=self.routesData[rr].trips[kk];
                                if (trip.statusInfo != 3 && info.indexOf(trip.time)<0) {
                                    trip.statusInfo = 2;
                                    trip.tripType = "enhanced";
                                }
                            }
                        }
                    }
    ...
    renderGroups: function() {
        ...
        if (self.routesData != null) {
            var arr = self.getRoutesArray();
            for (var i = 0; i < arr.length; i++) {
                renderTable(arr[i]);
                $("#selectRoutes").append('<option value="' + arr[i].id + '" >' + arr[i].name + '</option>');
            }
    ...
    updateSelectTimes: function() {
        ...
        if (self.routesData[$("#selectRoutes").val()] != null) {
    ...
    bindEvents: function() {
        ...
        vev("#selectRoutes", 'change', function() {
        ...
        vev("#selectTimes", 'change', function () {
            var tripId = $("#selectRoutes").val();
        ...
        vev("#btnCancelTrip", 'click', function () {
            if ($(this).text() == "Hủy") {
                cancelTrip();
            } else {
                vRqs({
        ...
        vev("#btnUpdateTrip", 'click', function () {
            updateTrip();
        });

        vev("#busManager tr", 'click', function () {
        ...
        vev("a.btn-book", 'click', function () {
            ...
            setTimeout(function () {
                vbf('rlBKS', tripId == null ? {} : {
                    o: {
                        data: {
                            t: tripId,
                            d: moment(new Date($(".departure").datepicker("getDate"))).format("DD.MM.YYYY"),
                            h: tripTime != null ? tripTime.replace(":", ".") : ""
                        }
                    }
                });
            }, 50);
    ...
    sortObject: function (o, sortKey) {
        var sortArray = function (arr, sortKey) {
            if (sortKey == null) {
                return arr.sort()
    ...
    getRoutesArray:function(){
        var self=this;
        var arr = [];
        for (key in self.routesData) {
            if (self.routesData[key].id != null) {
                arr.push(self.routesData[key]);
            }
             
        }
        for (var i = 0; i < arr.length-1; i++) {
            for (var j = i + 1; j < arr.length; j++) {
                if (arr[i].displayOrder > arr[j].displayOrder) {
                    var tmp = arr[i];
                    arr[i] = arr[j];
                    arr[j] = tmp;
                }
