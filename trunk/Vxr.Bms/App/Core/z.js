...
$.rights = { // List all Rights :    {val: giá trị trong Info (vd: 1|112|1); dtr: giá trị attr data-right trong tag (vd: S1230)
    ...
    bUnPr: { val: '1|10070|1', dtr: '' }, // Unblock print
    mPhoiManager: { val: '', dtr: 'S110000' }, // Quản lý phơi
}
...
function vRqs(o, p, x) {
    ...
    return $.ajax(c).done(function (r, s, j) { // data, textStatus, jqXHR
        ...
        if (r.Result == 0 && r.Expired == 2) {
            //alert("Phiên làm việc đã hết hạn, vui lòng đăng nhập lại.");
            window.location.reload();
            return;
        }
        if (r.Result == -10) {
            alert("Bạn đang truy vấn quá nhiều, vui lòng nghỉ và nhâm nhi cafe.");
            window.location.reload();
            return;
        }
...
function vIsPhone(s) {
    ...
    //check if exist characters
    if (!s.match(/(^[0-9]*$)/g)) return false;

    // kiểm tra xem có config bỏ qua điều kiện đầu số đt hay không
    if (_dict._hasByPassPhoneNumber != undefined && _dict._hasByPassPhoneNumber) {
        return true;
    }
...
function Vm(o) {
    ...
    return {
        ...
        _ajaxUpdate: function (id, d, isDelete, cb, msgSucc, msgErr) {
            ...
            if (d.BaseId != undefined && d.Date != undefined && d.Time != undefined) {
                _t = d.BaseId;
                _d = moment(d.Date).format('DD-MM-YYYY');
                _h = d.Time;
            }
            ...
            vRqs(
                ...
                function (u, r, l, t) {
                    ...
                    FlatObj.FTripGetTrip = false;
                    if (vIsEstStr(_t)) {
                        $('#bksContent').vbooking("load", { t: _t, d: _d, h: _h });
                    }
                    else {
                        $('#bksContent').vbooking("load");
                    }
        ...
        _onBasicUpdate: function (id, afterClick) {
            ...
            $.each(gDx(o), function (idx, x) {
                ...
                if (x.name == 'Username') {
                    var str = v.match(v.match(/[A-Z]+/));
                    if (str != null && str.length > 0) {
                        isValid = false;
                        msg += "\n Tên tài khoản không được chứa ký tự in hoa. Vui lòng nhập lại !";
                        failArr.push(x);
                    }
                }
...
function vDtToStr(f, d) {
    ...
    switch (f) {
        ...
        case "yyyy-mm-dd hh:mm:ss":
            result = year + "-" + pad(month) + "-" + pad(day) + " " + pad(hour) + ':' + pad(minute) + ':' + pad(second);
            break;
        case "hh:mm:ss":
...
function vPrsDt(d) {
    ...
    try {
        ...
        if (d == null) {
            //return new Date();
            var re = moment(new Date().toString(), "Asia/Ho_Chi_Minh");
            return re;
        }
        if (d.indexOf('Date') >= 0) { //Format: /Date(1320259705710)/    
            ...
            //var re = new Date(Date.parse(moment(d).format("llll")));
            //return re;
        } else if (d.length == 10) {
...
function vEscapeSearchString(text) {
...
Pager.prototype.getStartItem = function () {
    return (this.currentPage - 1) * this.itemsPerPage;
}

Pager.prototype.getEndItem = function () {
    return Math.min(this.currentPage * this.itemsPerPage, this.totalRecords) - 1;
}
