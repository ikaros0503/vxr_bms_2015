...
function rpCustomerByTrip(obj, els) {
    ...
    if ($.inArray(obj._cf.reportId, rpIdNeedCompId) >= 0)
        obj._cf.compId = compId;
};
...
function rpStaffByPickupDate(obj, els) {
    ...
    obj._cf = {
        ...
        mColMergForSubSubSumRow: 4, //Number of column merge for sub total row
...
function rpAgentOrBranchBySoldDate(obj, els) {
    ...
    var branch = $('#' + els[3]).val();
    var branchName = $('#' + els[3]).find('option:selected').text();
    var branchChargeId = $('#' + els[3]).find('option:selected').val();
    var header = "Từ ngày: " + fromDate + " - 00:00  &#x2192;  " + toDate + " - 23:59";
    ...
    var strW = " ((ChargeDate is null and '" + toFromDateDb(fromDate) + "'<=IsPrgCreatedDate and IsPrgCreatedDate<='" + toToDateDb(toDate) + "') or " +
                ...
                " and ((Status = 2 and (SUBSTRING(PaymentInfo, 0, CHARINDEX(':', PaymentInfo)) = '1' or SUBSTRING(PaymentInfo, 0, CHARINDEX(':', PaymentInfo)) = '6')))" +
                //(branch != '' ? (" and BranchChargeName = N'" + branchName + "' ") : ' ');
                (branch != '' ? (" and BranchChargeId = " + branchChargeId + " ") : ' ');
...
function rpRevenueByTripTTT(obj, els) {
    var fromdate = $('#' + els[0]).val();
    var route = $('#' + els[1]).val();
    var routeName = $('#' + els[1]).find('option:selected').text();
    var time = $('#' + els[2]).val();
    ...
    obj._cf = {
        ...
        mArrArgSumary: [6, 11],
        reportId: 10891,
        strOrderBy: ' order by FareFormatted desc, BranchChargeName, TripIdName, TripDate, PaymentTypeName',
        hdValue: header,
        bShowSubSumary: 1,
        classSubRowSumGobal: 'danger-sub-total text-right',
        classRowSumGobal: 'danger-total text-right',
...
function rpRevenueByRoute(obj, els) {
    ...
    var strW = " '" + toFromDateDb(fromdate) + "'<=TripDate " +
                    ...
                    "and CompId = " + app.cid + (route != '' ? (' and TripId = ' + route) : ' ') + " AND Status != 1 AND Status != 8 and (Status <> 3 or (Status = 3 and CancelFee is not null and CancelFee > 0)) GROUP BY TripIdName, BranchChargeName";
    ...
    obj._cf = {
        ...
        where: strW,
        //query: ["select TripIdName, BranchChargeName," +
...
function rpBranchByRoute(obj, els) {
    ...
    var strW = " '" + toFromDateDb(fromdate) + "'<=TripDate " +
        "and TripDate<='" + toToDateDb(todate) + "' " +
        "and CompId = " + app.cid + (route != '' ? (' and TripId = ' + route) : ' ') +
        (branch != '' ? (" and BranchChargeId='" + branch + "' ") : " ") + " AND Status != 1 AND Status != 8 " +
        "and (Status <> 3 or (Status = 3 and CancelFee is not null and CancelFee > 0)) " +
        "GROUP BY TripIdName, BranchChargeName";
    ;
    obj._a = 'rpRevenueByRoute';
    obj._cf = {
        bShowIndexRow: false,
        ...
        mArrArgSumary: [3, 4, 5, 6],
        reportId: 108,
        strOrderBy: ' order by TripIdName, BranchChargeName asc ',
        hdValue: header,
        classRowSumGobal: 'danger-total text-right',
        classSubRowSumGobal: 'danger-sub-total text-right',
        classListItem: 'success-total text-right',
...
function rpVexereRevenue(obj, els) {
    ...
    var strW = " '" + toFromDateDb(fromdate) + "'<=TripDate " +
            ...
            " AgentId in (74, 135) and (Status = 2 or (Status = 1 and ExpiredTime is null)) ";
