///#source 1 1 /App/Bu/Co.Bks/Model/Ticket/o.js
/*************************************************************************************
* TICKET 
...
var T = function (id, dept, cid, cname, cphone, status, issue, pdate, fare, note,
                    ...
                    cancelInfo, type, chargeDate, sNote, responUser, porDate,seatCode,ptime, staffName, cancelType, numOfSend, firstUserUpdated,
...
///#source 1 1 /App/Bu/Co.Bks/Model/Ticket/p.js
...
T.prototype._renderVCTicket = function (i) {
    var self = this;
    //Prepare data
    var cancelledDate = self._getCanceledDate();

    if (self._pdate.length <= 9)
        self._pdate = '0' + self._pdate;
    ...
    var data = {
        "ticket": {
            ...
            _pTime: self._ptime,
            _code: self._code,
            _seatCode: self._seatCode,
            _cusInfo: self._cname,
            _cDate: !$.isEmptyObject(cancelledDate) ? cancelledDate.toFormatString('dd-mm-yyyy hh:ii') : "",
            _cInfo: self._cancelInfo,
            _cUser: self._cuser,
            _cNote: self._sNote,
            _feature: "chưa có"
        }
    ...
    //Rendering
    var $html = $(vtpl(_dict._cTplTbData, data));
    return $html;
};
        ...
        if (vIsEstStr(self._hInfo)) {
            ...
            for (var i = 0; i < h.length; i++) {
                ...
                if (vIsEstStr(ht[6])) {
                    ...
                    var dt = "";
                    $.each(d, function (k, v) {
                        if ((_dict._historyKeys.hasOwnProperty(k) && vIsEstStr(v)) || k.indexOf('Fare') >= 0) {
    ...
    switch (k) {
        ...
        case "PaymentInfo":
            if (v != null) {
                var ifo = v.split(':');
                if (v == " ") {
                    result = "Hủy thanh toán";
                }else if (ifo.length > 0) {
                    $.each(_dict._pm, function (_, p) {
                        if (result == "") {
                            if (p[0] == ifo[0]) {
                                result = p[3];
