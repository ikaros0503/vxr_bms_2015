...
var _dict = {
    ...
    _sg: ["col-sm-12", "col-sm-4", "col-sm-4", "col-sm-4", "vcol-sm-5", "col-sm-2"],
    ...
    _err: [
        ...
        "Đã có ghế được thanh toán, vui lòng kiểm tra lại !"
    ],
    _pm: [
    ...
    _uForm: [
        ...
        [
            ...
            //[8, 1, "Điểm đi", "select", "Chọn điểm đi", "FromStop", "form-control", "", {}, [], ""],
            //[8, 2, "Điểm đến", "select", "Chọn điểm đến", "ToStop", "form-control", "", {}, [], ""],
    ...
    _filterDate: "<div class='col-md-2 col-sm-2 col-xs-6 pl0 ngay-chay' style='position: absolute;margin-left: 17%;'>" +
                            "<div class='form-group'>"+
                                "<div class='input-group'>"+
                                    "<a class='input-group-addon' style='cursor:pointer;text-decoration: none;' name='PreDate'>"+
                                        "&lt;"+
                        "</a>"+
                        "<input type='text' class='form-control input-sm datepicker hasDatepicker' name='DepartureDate' id=''>"+
                        "<a class='input-group-addon' style='cursor:pointer;text-decoration: none;' name='NexDate'>"+
                            "&gt;"+
                        "</a>"+
                        ...
                        "</div>"+
                        "</div>",
    _filterTime: "<div class='col-md-2 col-sm-2 col-xs-6 gio-chay' style='position: absolute;margin-left: 33%;'>" +
                            "<div class='form-group'>"+
                                "<select class='form-control input-sm' name='TimeSlot'></select>"+
                            "</div>"+
                        "</div>",
    ...
    // có ràng buộc điều kiện số điện thoại hay không
    _hasByPassPhoneNumber: false,
    _pnp: {
    ...
    _specialPos: {},
    //_sTpl: "<li class='seat {seat._half}' data-ticket-code='{seat._code}' data-position='{seat._coach}_{seat._row}_{seat._col}'><div class='{seat._class}'><div class='clearfix'><div class='seat-code'>{seat._label}</div><div class='pick'>{seat._pInfo}</div><div class='note'>{seat._note}</div></div>"
    ...
    _cTpl: "<li class='ext-seat'><div class='voseat vcseat'><div class='vocode'>{ticket._code}</div><div class='vinfo'><p class='timeValid'>{ticket._cDate}</p><p class='nameValid'>{ticket._cname}</p><p class='phoneValid'>{ticket._cphone}</p></div></div></li>",
    
    _cTplTb: "<div class='table-responsive'>"+
                                "<table class='table table-striped table-bordered table-hover' id='dataTables-example'>"+
                                    "<thead>"+
                                        "<tr class='titleTable'>" +
                                            "<th class='center'>STT</th>" +
                                            "<th class='center'>Ngày đi</th>" +
                                            "<th class='center'>Giờ đi</th>" +
                                            "<th class='center'>Mã vé</th>" +
                                            "<th class='center'>Mã ghế</th>" +
                                            "<th class='center'>Thông tin khách</th>" +
                                            "<th class='center'>Ngày giờ hủy</th>" +
                                            "<th class='center'>Lý do</th>" +
                                            "<th class='center'>Người hủy</th>" +
                            ...
                            "</div>",
    _cTplTbData:"<tr>"+
                                            "<td class='center'>{ticket._STT}</td>" +
                                            "<td class='center'>{ticket._pDate}</td>" +
                                            "<td class='center'>{ticket._pTime}</td>" +
                                            "<td class='center'>{ticket._code}</td>" +
                                            "<td class='center'>{ticket._seatCode}</td>" +
                                            "<td class='center'>{ticket._cusInfo}</td>" +
                                            "<td class='center'>{ticket._cDate}</td>" +
                                            "<td class='center'>{ticket._cInfo}</td>" +
                                            "<td class='center'>{ticket._cUser}</td>" +
                                            "<td class='center'>{ticket._cNote}</td>" +
                                            "<td class='center'>{ticket._feature}</td>" +
                                        "</tr>",
    _cTplTbJs:"<script>"+
    ...
    // bỏ qua sơ đồ ghế khi đếm số ghế trống chỗ khung chọn giờ
    _denySeatTempId: [],
    ...
    // ràng buộc điều kiện nơi đi, nơi đến
    _isValidateFromToStop: false,

    // không hiển thị chọn chặng con trong form Update
    _noShowRoutePoints: false,

    // AgentId: country code (Vietnam: vn, Cambodia: kh, Laos: la)
    ...
    phoiInList : {
        fields: [
            {name:"seatCode", label:"Mã ghế", align:"left"},
            {name:"name", label:"Tên", align:"center"},
            {name:"phone", label:"Số điện thoại", align:"left"},
            {name:"status", label:"Trạng thái", align:"center"},
            {name:"fare", label:"Giá vé", align:"right"},
            {name:"toName", label:"Nơi đến",align:"center"}
        ],
        groupBy: "phone",
        orderBy: "seatCode",
        pickupList: true,
