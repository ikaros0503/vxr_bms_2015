...
(function ($) {
    ...
    $.extend(true, $.custom.vbooking.prototype, {
        ...
        getInfoObject: function () {
            ...
            var driverInfo = $.trim(me._data[me._cTripIndex].TripDriver);
            if (driverInfo != null && driverInfo != "") {
                var drivers = driverInfo.split("~");
                for (var l = 0; l < drivers.length; l++) {
                    if (drivers[l] != null && drivers[l].indexOf(me._cTripTime) >= 0) {
                        e.phoi.driverInfo = drivers[l].replace(me._cTripTime + "|", "");
                    }
                }
            }
            e.phoi.tripNote = "";
            if (me._data[me._cTripIndex].Ts[me._cTripTime] != null && me._data[me._cTripIndex].Ts[me._cTripTime].length > 0) {
                e.phoi.tripNote = me._data[me._cTripIndex].Ts[me._cTripTime][0].N;
            }
                            ...
                            for (var i = 0; i < numCol; i++) {
                                if (typeof rx[i] != "undefined" && rx[i] != null) {
                                    ...
                                    if (!$.isEmptyObject(t)) {
                                        ...
                                        s.staffName = t._staffName != null ? t._staffName : "";
                                        s.createdUser = t._cuser != null ? t._cuser : "";
                                        s.chargedUser = t._suser != null ? t._suser : "";
                                        
                                        s.paymentActorDisplay = s.staffName;
                                        if (t._pmInfo != null && t._pmInfo.indexOf("Đại lý") >= 0) {
                                            s.paymentActorDisplay = getSubItem(t._pmInfo, "|", 2);
                                        }
                                        ...
                                        s.ticketsCountGtZeroG = "-" + s.ticketsCountGtZero + "G";
                                            ...
                                            if (uniqueCustomers.length > 0) {
                                                for (var kk = 0; kk < uniqueCustomers.length; kk++) {
                                                    if (me.isTwoSeatPerfectTogether(t, uniqueCustomers[kk], _dict.groupSeat.groupFields)) {
                                                        ...
                                                        s.groupSeatDisplay = "";
                                                        s.seatDisplay = "hide";
                                                        s.labelAndTicketsCount = s.label + " " + s.ticketsCount + "G";
                                                        s.cusNameAndTicketsCount = s.customerName + " " + s.ticketsCount + "G";
        ...
        _printBKS: function (keyPrint) {
            ...
            if (vIsEstStr(keyPrint) && keyPrint == 'printInList') {
                this.printList();
                return;
            }
            else if (vIsEstStr(keyPrint) && keyPrint == 'in-danh-sach') {
        ...
        printList: function () {
            var config = _dict.phoiInList;
            var me = this;
            if (config == null) return;
            
            var $printContent = $('<div />').addClass('print-container')
            $printContent.append(me._renderBKSTitle());
            $printContent.append(me.inDanhSach());
            $printContent.append(config.pickupList ? this._renderPrintPickupTemplate().addClass('list').css('page-break-before', 'always') : "");
            $printContent.append(config.tripSummary ? this._renderPrintTripInfo() : "");
            
            FlatObj.RBePrint = false;
            var printData = {
                title: 'BookingSheet' + (new Date()).getTime(),
                html: $printContent.html(),
                css1: _dict._pStyleUrlDefault,
                css2: _dict._pStyleUrl
                //css1: "http://localhost:2015/bms/Content/css/vbooking-print-default.css",
                //css2: "http://localhost:2015/bms/Content/css/vbooking-print-default.css"
            };

            var printWindow = window.open(
                'about:blank',
                printData.title,
                'width=' + screen.width + ', height=' +screen.height + ', scrollbars=1');

            printWindow.document.write(vtpl('<html><head><title>{title}</title>' +
                        '<link rel="stylesheet" href="{css1}" type="text/css" />' +
                        '<link rel="stylesheet" href="{css2}" type="text/css" /></head>' +
                    '<body>{html}</body></html>', printData));
            printWindow.focus();
        ...
        _renderBKSTitle: function () {
            return $('<div />').addClass('row').append($(vtpl(_dict._pTitleTpl, this.getTitleObject())));
        },
                            ...
                            if (typeof r != "undefined") {
                                $.each(r, function (is, s) {
                                    if (typeof s != "undefined" && s != null) {
                                        ...
                                        if (!$.isEmptyObject(t)) {
                                            var fromStop = { Id: "", Type: "", Code: "", Name: "" };
                                            var toStop = { Id: "", Type: "", Code: "", Name: "" };
                                            if (vIsEstStr(t._fromStop)) {
                                                var indFromStop = t._fromStop.indexOf('~');
                                                var fromStops = t._fromStop.substr(indFromStop + 1, t._fromStop.length).split("|");
                                                fromStop = {
                                                    Id: fromStops[0],
                                                    Type: fromStops[1],
                                                    Code: fromStops[2],
                                            ...
                                            if (vIsEstStr(t._toStop)) {
                                                var indToStop = t._toStop.indexOf("~");
                                                var toStops = t._toStop.substr(indToStop + 1, t._toStop.length).split("|");
                                                toStop = {
                                                    Id: toStops[0],
                                                    Type: toStops[1],
                                                    Code: toStops[2],
        ...
        inDanhSach: function () {
            ...
            //seatCode, name, phone, pText, note, status, seri, createdUser, chargedUser, fare, fromCode, fromName, toCode, toName
            config = {
                fields: [
                    {
                        name:"seatCode",
            ...
            var fromCode = "", fromName = "", toCode = "", toName = "";
            var $table = $(_dict._pclTpl);

            var $th = $('<tr />');
            for (var i = 0; i < config.fields.length; i++) {
                $th.append($("<th></th>").text( config.fields[i].label).addClass("text-" + config.fields[i].align));
            }
            $table.addClass("main");
            $table.find("thead").empty().append($th);
            var $tbody = $table.find('tbody');
                                ...
                                if (typeof s != "undefined" && s != null) {
                                    ...
                                    if (!$.isEmptyObject(t)) {
                                        ...
                                        tickets.push({
                                            ...
                                            pText: pText,
                                            note: t._note,
                                            status: t._status == 1 ? "Đặt chỗ" : "Đã TT",
                                            _class: "",
                                            seri: t._seri,
                                            createdUser: t._cuser,
                                            chargedUser: t._suser,
                                            fare: t._fare.toMn() + "đ",
                                            fromCode: fromCode,
                                            fromName: fromName,
                                            toCode: toCode,
            ...
            var compareSeats = function (seat1, seat2, condition) {
                if (!seat1 || !seat2 || !seat1[condition] || !seat2[condition]) return false;
                return seat1[condition]==seat2[condition];
            };

            var orderSeats = function (seat1, seat2, condition) {
                console.log(seat1[condition], seat2[condition], seat1[condition] >= seat2[condition]);
                if (!seat1 || !seat2 || !seat1[condition] || !seat2[condition]) return false;
                return   seat1[condition] >= seat2[condition];
            };
            ...
            if (tickets.length > 0) {
                for (var i = 0; i < tickets.length; i++) {
                    if (sTickets.length > 0) {
                        var isFound = false;
                        for (var j = 0; j < sTickets.length; j++) {
                            if (compareSeats(tickets[i],sTickets[j], config.groupBy)){
                                sTickets[j].seatCodes.push(tickets[i].seatCode);
                                sTickets[j].seatCode = sTickets[j].seatCode + ", " + tickets[i].seatCode;
                                isFound = true;
                                break;
                            }
                        }
                        if (!isFound) {
                            sTickets.push(tickets[i]);
                        }
                    } else {
                        sTickets.push(tickets[i]);
                    }
                ...
                if (sTickets.length>1){
                    for (var i = 0; i < sTickets.length-1; i++) {
                        for (var j = i+1; j < sTickets.length; j++) {
                            if (orderSeats(sTickets[i], sTickets[j], config.orderBy)) {
                                var tmp = sTickets[i];
                                sTickets[i] = sTickets[j];
                                sTickets[j]=tmp;
                            }
                        }
                    }
            ...
            if (sTickets.length > 0) {
                var index = 1;
                for (var i = 0; i < sTickets.length; i++) {
                    var $tr = $("<tr></tr>");
                    for (var j = 0; j < config.fields.length; j++) {
                        $tr.append($("<td></td>").text(sTickets[i][config.fields[j].name]).addClass("text-" + config.fields[j].align));
                    }
                    $tbody.append($tr);
                }
            } else {
                $tbody.append($('<tr />').append($('<td />').attr('colspan', '16').text("Chưa có hành khách đặt vé chuyến xe này")));
            }
