...
define({
    start: function (o) {
        ...
        vbv('doShowCancelForm', function (e) { // show cancel form
            ...
            var $form = $(o._e);
            var hasBookingSeat = function (st) {
                if (st == null || st.length <= 0) return false;
                for (var i = 0; i < st.length; i++) {
                    if (st[i]._getCurrentTicket()._status == 1) return true;
                }
                return false;
            };
            if (hasBookingSeat(e.d.st)) {
                $form.find('.divFeePercentRadio').children().prop('disabled', true);
                $form.find('.divFeePercentRadio div').children().prop('disabled', true);
            } else if (typeof e.d.allIsBooking != 'undefined' && e.d.allIsBooking) {
                $form.find('.divFeePercentRadio').children().prop('disabled', true);
                $form.find('.divFeePercentRadio div').children().prop('disabled', true);
            } else {
    ...
    _createCancelDialogDiv: function () {
        ...
        // Khởi tạo thêm lý do hủy vé hoàn toàn cho các tài khoản có quyền
        if ($.rights.cReDeFo != null && app.rights.indexOf($.rights.cReDeFo.val) != -1) {
            $cancelBody.find('select[name="CancelReason"] option:last-child').before($('<option value="4">Hủy hoàn toàn</option>'));
        }
    ...
    _bindEventOnCancelForm: function (f, tId, tName, tDate, st, tBus, sc) {
        ...
        //    e.preventDefault();
        //});
        f.find('[name="CancelFeeMoneyInput"]').on('focus', function (e) {
            $(this).select();
            f.find('input[name="FeeMoneyRadio"]').prop('checked', true);
            f.find('input[name="FeePercentRadio"]').prop('checked', false);
        }).mouseup(function (e) {
        ...
        f.find('[name="CancelFeePercentInput"]').on('focus', function (e) {
            $(this).select();
            f.find('input[name="FeeMoneyRadio"]').prop('checked', false);
            f.find('input[name="FeePercentRadio"]').prop('checked', true);
        }).mouseup(function (e) {
        ...
        f.find('button.btn-cancel').unbind().on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
            try {
    ...
    _cancelTicket: function (f, isPercent, cancelFeePercent, isMoney, cancelFeeMoney, cancelReasonId, cancelReasonInfo, tId, tName, tDate, st, tBus, sc) {
        ...
        var cb = function (u, r, l, t) {
            ...
            vbf('resetSeatStack'); // Reset seat stack
            vbf('reloadBookingSheet', {}, function () { vbf('doGetTotalSeats'); }); // Reload sheet
