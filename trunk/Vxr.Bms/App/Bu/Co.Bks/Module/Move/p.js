...
define({
    ...
    _maskMoveTicket: function (tId, tName, tDate) {
        var me = this;
        // updated by Duy 2015-08-26 16:20:00
        app.allowMoving = true;
        //Set is moving
        app.movingStack = [];
        app.fMoving = true;
    ...
    _createGetTicketBySeatObj: function (seatCode, tripId, tripDate, stageCode) {
        var obj = {};
        obj._a = 'fGetTicketBySeat';
        obj._c = {
            seatCode: seatCode,
            tripId: tripId,
            tripDate: moment(tripDate).format('YYYY-MM-DD HH:mm'),
            stageCode: stageCode,
        };
        obj._f = '*';
        return obj;
    ...
    _moveTicket: function (seat, tId, tName, tDate, tBus, tStage, fromId, toId, stopPoints) {
        var me = this;
        var seatCode = seat._label + '|' + seat._coach + '|' + seat._row + '|' + seat._col;
        ...
        var obj = me._createGetTicketBySeatObj(seatCode, tId, tDate, tStage);
        vRqs(obj, function (u, r, l, t) {
            if (u == 1) {
                if (r[0][0] != null) return; //Kiem tra xem co ve da dat o seat chua (loai tru StageCode = 1)
                app.allowMoving = true;
    ...
    _createMovedTicketObj: function (seat, tId, tDate, tBus, fId, tStage, fromId, toId, stopPoints) {
        ...
        var fromArea = "", toArea = "";
        if (vIsEstStr(movedTicket.fromArea)) {
            fromArea = movedTicket.fromArea;
        } else {
        ...
        if (vIsEstStr(movedTicket.toArea)) {
            toArea = movedTicket.toArea;
        } else {
