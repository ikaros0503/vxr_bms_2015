    ...
    $.extend(true, $.custom.vbooking.prototype, {
        ...
        *************************************************************************/
        _create: function () {
            ...
            var self = this;
            vbv("updateFilters", function (e, model) {
                            ...
                            $.each(c, function (__, s) {
                                if (typeof s != "undefined" && s != null) {
                                    ...
                                    if (!$.isEmptyObject(t)) {
                                        ...
                                        var tcode = t._code;
                                        if (cphone == keyword) {
                                            var obj = self._$cancelSheet.find('.seat[data-position="' + s._coach + '_' + s._row + '_' + s._col + '"][data-ticket-code="' + tcode + '"]');
                                            if (!self._grid) {
                            ...
                            $.each(c, function (__, s) {
                                if (typeof s != "undefined" && s != null) {
                                    ...
                                    if (!$.isEmptyObject(t)) {
                                        ...
                                        if (cCode == keyword) {
                                            var obj = self._$cancelSheet.find('.seat[data-position="' + s._coach + '_' + s._row + '_' + s._col + '"][data-ticket-code="' + cCode + '"]');
                                            if (!self._grid) {
                                                obj = self._$cancelSheet.find('.lseat[data-position="' + s._coach + '_' + s._row + '_' + s._col + '"]');
                                            }
                                            self._updateSeatStack(obj, s);
        ...
        _bindEventOnFilterForm: function () {
            ...
            $form.find('button').each(function (_, b) {
                if ($(b).hasClass('btn-print')) {
                    ...
                    vev(b, 'click', function (e) {
                        e.preventDefault();
                        if ($(b).hasClass('closed')) {  //hủy xuất bến (Editor: Khánh)
                            me._cancelClosedStatus();
                        } else {
                ...
                } else if ($(b).hasClass('btn-huy-xuat-ben')) {
                    vev(b, 'click', function (e) {
                        e.preventDefault();
                        $('button.btn-thong-ke').removeClass('active');
                        me._cancelClosedStatus();
                ...
                //else if ($(b).hasClass('btn-moveSeats')) {
                //    vev(b, 'click', function (e) {
                //        e.preventDefault();
                //        vfe('body', 'onMoveSeats', {
                //        });
                //    });
                //}
            });

            vbv('fTripChange', function (e) {
                //Update Status
                var newTripId = e.d.value;
                var $jtr = $("tr.jtable-data-row[data-record-key='" + newTripId + "']");
            ...
            vbv('doSearchResultSelected', function (e) {
                ...
                var tripTimeRecord = d.tTime, phoneRecord = d.phone, isCanceled = d.isCanceled, code = d.code;
...
$(document).mouseup(function (e) {
    ...
    var button = $("div.searchContainer > span");
